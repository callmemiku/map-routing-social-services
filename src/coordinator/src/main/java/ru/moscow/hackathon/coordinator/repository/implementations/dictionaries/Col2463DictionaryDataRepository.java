...
public class Col2463DictionaryDataRepository implements CoordinatedRepository {

    JdbcTemplate jdbcTemplate;

    @Override
    public void handle(OperationType type, List<String[]> rows) {
        log.debug("[COL2463 DICT REPO] saving {} records", rows.size());
        Mono.just(rows)
                .map(it ->
                        jdbcTemplate.batchUpdate(
                                """
                                        INSERT INTO col_2463_data (
                                            id, 
                                            code,
                                            description
                                        ) VALUES (?, ?, ?)
    ...
    public OperationType myType() {
        return OperationType.COL_2463_DICTIONARY;
    }
}
