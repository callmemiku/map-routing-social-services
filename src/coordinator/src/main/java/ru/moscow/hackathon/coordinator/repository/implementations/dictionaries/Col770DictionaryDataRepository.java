...
public class Col770DictionaryDataRepository implements CoordinatedRepository {

    JdbcTemplate jdbcTemplate;

    @Override
    public Mono<List<Pair<UUID, List<String>>>> handle(OperationType type, List<String[]> rows) {
        log.debug("[COL770 DICT REPO] saving {} records", rows.size());
        return Mono.just(rows)
                .map(it ->
                        jdbcTemplate.batchUpdate(
                                """
                                        INSERT INTO col_770_data (
                                            id, 
                                            code,
                                            description
                                        ) VALUES (?, ?, ?)
    ...
    public OperationType myType() {
        return OperationType.COL_770_DICTIONARY;
    }
}
